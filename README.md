...
| 早期生产样本（一周） | 付费生成提交 | 局域网使用账号 | 一条 22 秒换景片的生成成本 |
|:--:|:--:|:--:|:--:|
| **92** | **109** | **10** | **¥17.6** |

一条 15 秒的鞋类带货视频，重拍一次的成本是模特、场地、摄影、剪辑。在已测样片中，这套系统把生成成本压到**十几元量级**，减少重新组织拍摄的工作。上表为历史实测样本，不代表所有任务的成本和耗时。
...
| **创意雷达** | 持续发现同类目热门与跨类目灵感 | 多源采集 + 热度快照 + 画面筛选 + 原片预览与反推 |
| **一键创意** | 从产品实图生成多个短视频创意方向 | 笑点 / 反转 / 产品记忆点 → 确认方案 → 动作编排 → Prompt |
| **视频反推与裂变** | 把参考片的创意结构适配到新产品 | 原生视频理解 + 动作拆解 + 图片槽位绑定 + 关联元素适配 |
...
![创意雷达：跨平台内容发现、公开热度指标、原片预览与一键反推](assets/ui-creative-radar.png)

**实现逻辑：** 按类目、关键词、市场与时间窗口采集 TikTok / Instagram 内容，结合公开播放和互动指标、增量快照、画面审核与创意可迁移性组织信息流；原片下载后可继续分析动作、镜头与声音线索。

**核心能力：** 从同类目热门到跨类目灵感，把“发现 → 筛选 → 看原片 → 反推 → 换品制作”放进同一个工作台。把碎片化刷素材变成持续积累、可以直接推进生产的创意素材库，减少团队在平台、表格和制作工具之间反复搬运。

### 创意生成

![创意生成器：慈爱六视图绑定图片编号，反推并编辑四秒创意方案](assets/ui-creative-generator.png)

**实现逻辑：** Seed 2.1 Turbo 读取产品图并生成创意；上传参考片时通过视频接口理解剧情，再把内容拆成主体、前态、动作、接触关系与后态。确认方案后编排 Prompt，用 @图片绑定真实产品，并接入 Seedance 2.5 生成。

**核心能力：** 没有原片也能从产品一键获得多个创意方向；有原片则可以反推、定向修改和裂变。换品时会一起适配服务于产品的色彩、道具和效果，将“想创意、写提示词、对齐产品”串成连续工作流。用户负责选择与确认，AI 承担构思和拆解。

**图中案例：** 将“注入液体后显色”的创意适配到粉色「慈爱」，六张产品图明确外观来源，保留 4 秒动作编排。左侧为参考原片，右侧为产品绑定与已保存 Prompt。
...
**创意链路：** 创意雷达负责发现与筛选；Seed 2.1 Turbo 负责创意、视频理解与动作拆解；产品参考图和 Prompt 一起进入 Seedance 2.5。方案、绑定版本与生成状态保存到台账，成片可继续交给 Topaz 增强。
...
- **本机素材归属与按需云端处理**：每台设备的素材由 Local Agent 管理；调用云端理解或生成时，按任务链路临时上传必要素材，部分服务会中转媒体，完成后按保留策略清理。
...
| 项目 | 单价 | 实测 |
|---|---|---|
| gpt-image-2 历史图像样本：输入 / 输出 | $8 / $30 per M tokens | 输出 ≈ 1,300 tokens/帧 |
...
**本次新增** —— 创意雷达信息流、同类目与跨类目发现、原片与公开指标预览、视频反推、一键创意、动作拆解、产品关联元素适配、多视角 @图片绑定、创意裂变、方案编辑与历史、Seedance 生成入口及任务状态。

**已有生产能力** —— 视频入库、自动切镜头、关键帧抽取、gpt-image 换品/换人/换景、Seedance 2.5 编辑模式生成、逐镜显式绑定、显式任务型、存在锁、换语种双通路、画面字幕检测/翻译/重烧、比例转换、Topaz 本地增强、剪映草稿产出、成本核算与用量台账、提交对账、账号分区与局域网分发、崩溃恢复与开机自启、数据保留策略。

**已实现、暂未启用** —— 视觉模型自动判展示面（侧面判断 12/12 正确，¥0.0055/帧），实测效率不如人工点选，保留代码不接线。

**持续优化** —— 原创创意的笑点与产品表达、复杂动作的生成一致性、反推审核准确性、平台采集稳定性； 换景全长稳定性（逐镜显式绑定在 7 镜片段 7/7，全长 12 镜待复核；若后半段仍失守，按全局规则分段并配帧精确拼接与逐镜自动验收）；旧字幕擦除服务接入。

**规划** —— 白模驱动（把参考视频换成不含原片外观的运动代理，从结构上消灭场地回退）；投放表现反馈与创意排序闭环；3D 重建替代固定角度产品照。
